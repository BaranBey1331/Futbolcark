...
class _WheelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final overlay = Paint()
      ..shader = radial.createShader(Rect.fromCircle(center: center, radius: radius));
...
class ResultOverlay extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final double w = min(MediaQuery.of(context).size.width * 0.86, 420.0).toDouble();
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(valueBig, style: const TextStyle(fontSize: 64, fontWeight: FontWeight.w900)),
          ],
        ),
      ),
    );
...
class PlayerCardView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double w = min(MediaQuery.of(context).size.width * 0.80, 360.0).toDouble();
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(
          ...
          colors: [Color(0xFF6B747B), Color(0xFF2D3338), Color(0xFF7E878D)],
        ),
        boxShadow: const [BoxShadow(blurRadius: 22, color: Colors.black54)],
      ),
      child: Column(
        children: [
          Row(
            children: [
              Text('${career.overall}', style: const TextStyle(fontSize: 46, fontWeight: FontWeight.w900)),
              const SizedBox(width: 10),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(career.positionShort, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 10),
                  _flagGermany(),
                ],
              ),
          ...
          // positionLabel yok → enum’daki TR etiketi
          Text(p.position.tr, style: const TextStyle(color: kTextSoft, fontWeight: FontWeight.w800)),
...
class _GlassFieldShell extends StatelessWidget {
  const _GlassFieldShell({required this.label, required this.child});
  final String label;
  final Widget child;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.white70)),
